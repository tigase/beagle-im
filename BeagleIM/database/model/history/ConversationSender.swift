...
typealias ConversationSenderProtocol = ConversationSender

enum ConversationSender: Equatable {
    ...
    static func me(conversation: ConversationKey) -> ConversationSender {
        return .buddy(nickname: AccountManager.getAccount(for: conversation.account)?.nickname ?? conversation.account.stringValue)
    }
    
    static func buddy(conversation: ConversationKey) -> ConversationSender {
