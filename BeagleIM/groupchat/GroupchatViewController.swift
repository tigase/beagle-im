...
class GroupchatViewController: AbstractChatViewControllerWithSharing, NSTableViewDelegate, ConversationLogContextMenuDelegate, NSMenuItemValidation {
    ...
    override func prepareConversationLogContextMenu(dataSource: ConversationDataSource, menu: NSMenu, forRow row: Int) {
        ...
        if let item = dataSource.getItem(at: row) {
            ...
            if room.state == .joined {
                if item.state.direction == .incoming && item.payload != .retraction && room.roomFeatures.contains(.messageModeration) && room.role == .moderator {
    ...
    @objc func moderateMessage(_ sender: NSMenuItem) {
        ...
        alert.beginSheetModal(for: self.view.window!, completionHandler: { result in
            ...
            case .alertFirstButtonReturn:
                Task {
                    do {
                        try await chat.moderate(entry: item);
                    } catch {
                        self.showError(message: NSLocalizedString("Moderation failed!", comment: "message retraction failed alert title"), error: error)
                    }
                }
            default:
                break;
