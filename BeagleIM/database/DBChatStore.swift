...
open class DBChatStore: ContextLifecycleAware {
    ...
    public let conversationDispatcher = DispatchQueue(label: "ConversationDispatcher", attributes: .concurrent)
...
//    }
    ...
    public func close(conversation: Conversation) -> Bool {
        return queue.sync {
            ...
            // FIXME: incorrect parameter count when closing chat?
        ...
        queue.async {
            ...
            let conversations = try! Database.main.reader({ database in
                return try database.select(query: .chatFindAllForAccount, params: ["account": account]).mapAll({ cursor -> Conversation? in
                    ...
                    let lastMessageEncryption = DBChatHistoryStore.encryptionFrom(cursor: cursor, encryptionKey: "lastEncryption", fingerprintKey: "lastFingerprint");//MessageEncryption(rawValue: cursor.int(for: "lastEncryption") ?? 0) ?? .none;
...
//        newActivity(, isUnread: <#T##Bool#>, for: <#T##any Conversation#>)
//    }
